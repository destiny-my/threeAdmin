        ...
        <dv-border-box-12 class="running-card-box">
          <el-row :gutter="0">
            <div class="card-box-power">
              <div class="card-title"><span>各用水类型占比</span></div>
            ...
            <el-row :gutter="0">
              ...
              <el-col :span="13">
                <TableSource :tableData="tableRatio" :header="headerRatio" :pStyle="showPage" :total="page.total" style="margin-top: 10%;"></TableSource>
              </el-col>
            </el-row>
          </el-row>
        </dv-border-box-12>
        ...
        <el-row class="pt-10 pl-10 title-fifth" :gutter="15">
          <el-col class="history-card-box-filter" :xs="4" :sm="4" :md="4" :lg="4" :xl="4">
            ...
            <el-radio-group v-model="radioDivisionMode" @change="updateNames">
              ...
              <el-radio label='water'>按用水类型</el-radio>
            </el-radio-group>
          </el-col>
          <el-col class="history-card-box-filter" :xs="4" :sm="4" :md="4" :lg="4" :xl="4">
            <span class="opacity-75">数据类型：</span>
            <el-radio-group v-model="radioData">
              <el-radio label="1">用水量</el-radio>
...
import {colorWaterProp, optionCurves, optionPie,} from "@/api/carbon/option";
...
export default {
  ...
  mounted() {
    ...
    this.updateRatio(this.dateRatio) // 更新用水类型碳占比
    this.updateNames() // 更新名称列表
    this.AlarmConfigList()
  },
  methods: {
    AlarmConfigList(){
      Analyse.getAlarmConfigList({dataType:"water"}).then((res) => {
        var arr_annotation=[]
        res.rows.map((item) => {
    ...
    updateWarn() {
      // 更新碳排警告
      Analyse.EnergyConsumptionAlarmData({dataType: 'water'}).then((res) => {
    ...
    updateRatio: function (date) {
      // 更新用水类型碳占比
      let cycleType = 'day', dataType = 'water', returnDatatype = 3, dayTime = null
      ...
      Monitor.getSpaceRatioData(cycleType, dataType, returnDatatype, dayTime).then((res) => {
        ...
        this.optionPieRatio.series[0].itemStyle.color = (params) => {
          return colorWaterProp[params.dataIndex]
        }
      ...
      Analyse.getCurvesData(data).then((res) => {
        ...
        for (const val of yAxis) {
          ...
          for (let i = 0; i < series.length; i++) {
            if (this.radioDivisionMode === 'water') {
              series[i].color = colorWaterProp[i]
            } else {
    ...
    resetForm: function () {
      // 初始化历史数据查询表单
      this.radioDivisionMode = 'water' // 历史数据-划分方式
